// モジュールをインポート
import * as dm from './modules/discordModule';
...
(async () => {
  try {
    await rest.put(dm.Routes.applicationGuildCommands(dm.clientId, dm.guildId), {
      body: commands,
    });
...
dm.discord.once('ready', () => {
  console.log(`準備が完了しました ${dm.discord.user?.tag}がログインします`);
});
...
(async () => {
  dm.discord.on(
    'interactionCreate',
    async (
      interaction: dm.Interaction | dm.AutocompleteInteraction | dm.ChatInputCommandInteraction
    ) => {
...
dm.discord.once('ready', () => {
  cron.schedule('0 0 8,18 * * *', async () => {
    const channel = dm.discord.channels.cache.get(dm.channelId);
